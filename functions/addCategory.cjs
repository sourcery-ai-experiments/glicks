...
const categorySchema = require(resolve(process.env.PROJECT_ROOT, 'src', 'schemas', 'categoriesSchema.json'));
const { categoriesSchemaValidation } = require(resolve(process.env.PROJECT_ROOT,'src','scripts','schemaValidation.cjs'));


const app = express();
app.use(cors());
app.use(json());
const filePath = join(process.env.PROJECT_ROOT, 'src', 'assets', 'data', 'categories.json');
...
const authenticate = (req, res, next) => {
  ...
  if (token === TOKEN) {
    console.log('UC 19 user authenticated');
    next();
  } else {
    res.status(403).json({ error: 'This action is for staff only! Please leave th kitchen!' });
  }
};

// API endpoint for updating categories
app.post('/api/addCategory', authenticate, (req, res) => {
  const newCategory = {
    name: null,
    checked: false,
    id: null,
